...
export function loadConfig(): AppConfig {
  try {
    const config: AppConfig = {
      ...
      amqp: {
        host: z.coerce.string().parse(process.env.AMQP_HOST),
        username: z.coerce.string().parse(process.env.AMQP_USER),
        password: z.coerce.string().parse(process.env.AMQP_PASSWORD),
      },
      db: {
